...
**Name each file by its shot number** (`1.png`, `2.mp4`, `3.mp4`, …), drop
them all in `~/Downloads/discoterminal-media/`, and tell Claude — the
conversion (mp4 → optimized GIF), renaming, and README updates are handled
from there.
...
- [ ] **CleanShot**: Record → select the kitty window → MP4. Keep clips
...
```sh
mkdir -p ~/Downloads/discoterminal-media   # drop 1.png … 16.mp4 here
```

Then say the word. Claude will: convert mp4s → optimized GIFs (10 fps,
900 px, 128 colors), rename to the final names above, add the two new
